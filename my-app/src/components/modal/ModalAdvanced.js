...
const ModalAdvanced = ({ children, heading, ...props }) => {
  return (
    // thừa hưởng tính năng của modal base
    <ModalBase {...props}>
      {/* <div className="bg-white w-full relative z-10 p-10 rounded-lg"> */}
      <span
        className="absolute top-0 right-0 flex items-center
    w-10 h-10 rounded-full bg-white p-1 justify-center
    cursor-pointer translate-x-2/4 -translate-y-2/4 transition ease-in-out 
    hover:bg-red-500 hover:scale-75"
      ...
      <h1 className="text-3xl mb-5 font-medium text-center text-black">
        {heading}
      </h1>
      {children}
      {/* </div> */}
